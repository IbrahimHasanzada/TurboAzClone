...
// B R A N D L A R I   C I X A R I R 
const uniqueBrands = new Set();
cars.forEach(item => {
    if (item.brand && !uniqueBrands.has(item.brand)) {
...
const uniqueMarka = new Set();
cars.forEach(item => {
    if (item.model && !uniqueMarka.has(item.model)) {
        uniqueMarka.add(item.model);
        model.innerHTML += `<option>${item.model}</option>`;
    }
});

const city = document.getElementById("seher")

const uniqueCity = new Set();
cars.forEach(item => {
    if (item.city && !uniqueCity.has(item.city)) {
...
const uniqueBan = new Set()
cars.forEach(item => {
    if (item.city && !uniqueBan.has(item.banType)) {
        uniqueBan.add(item.banType);
        ban.innerHTML += `<option>${item.banType}</option>`;
    }
});
...
cars.forEach(item => {
    if (item.currency && !uniqueVal.has(item.currency)) {
        uniqueVal.add(item.currency);
        val.innerHTML += `<option>${item.currency}</option>`;
    }
});

// M a s i n   I s t e h s a l   I l i

const ilmin = document.getElementById("il-min")
const ilmax = document.getElementById("il-max")
let kod = ''
...
// M a s i n l a r   k a r t l a r i    c i x a r 

const carsCard = document.getElementById("cars-card")
let tap = ''
function showModelOption() {
    model.classList.remove('model-sec')
    tap = `<option value="Model">Model</option>`
    model.innerHTML = tap
    let models = [...new Set(cars.filter((item) => item.brand == marka.value).map((item) => item.model))]
    for (let i = 0; i < models.length; i++) {
        tap += `<option>${models[i]}</option>`
...
cars.map((item) => {
    carsCard.innerHTML +=
        `
            <div class="card">
            <img src="${item.images}" class="card-img-top" alt="Cars">
            <div class="card-body">
            <h5 class="card-title">${item.price} ${item.currency}</h5>
            <p class="card-text">${item.brand} ${item.model}</p>
            <p class="card-text">${item.year} ${item.engine}L, ${item.odometer} ${item.odometerUnit}</p>
            <p class="card-text">${item.city}, Bu gün 17:38</p>
            </div>
        </div>
            `
})
...
function ShowCards(item) {
    ...
    const inpmin = document.getElementById("third-header-input-first")    
    const inpmax = document.getElementById("third-header-input-second")    
...
function showCars() {
    ...
    if (city.value !== '') {
        filtered = filtered.filter((car) => {
            return car.city === city.value
        })
    }
    if (ban.value !== 'ban') {
        filtered = filtered.filter((car) => {
            return car.banType === ban.value
        })
    }
    if (val.value !== '') {
        filtered = filtered.filter((car) => {
    ...
    if (ilmax.value !== 'max') {
        filtered = filtered.filter((car) => {
            return car.year < ilmax.value
        })
    }
    if (inpmin.value) {
        filtered = filtered.filter((car) => {
            return car.price > inpmin.value
        })
    }
    if (inpmax.value) {
        filtered = filtered.filter((car) => {
            return car.price < inpmax.value
        })
    }
    if(kredit.classList.contains('button-color')){
        filtered = filtered.filter((car) => {
            return car.credit
    
        })
    }
    if(barter.classList.contains('button-color')){
        filtered = filtered.filter((car) => {
            return car.barter
    ...
    if (filtered.length) {
        ...
        filtered.forEach((car) => {
            carsCard.innerHTML += ShowCards(car)
        })
    } else {
        carsCard.innerHTML = 'Sizin seçiminizə uyğun maşın yoxdur!'
    }
